...
#  read -p "press enter to continue...."
  cat "${ar}" | grep -e "<NO" -e "FPDM" -e "FPHM" | sed 's/<FPDM>//g' | sed 's/<FPHM>//g' | sed 's/<\/FPDM>//g' | sed 's/<\/FPHM>//g' | sed 's/	//g' | sed 's/ //g' >${req_file_name}"this.xml"
  dos2unix ${req_file_name}"this.xml">/dev/null 2>&1
  ch_line=`cat ${req_file_name}"this.xml" | wc -l`
  ...
  if [ $ch_flag -ne 0 ]; then
    ...
    rm -rf ${req_file_name}"this.xml"
    #mv
    break 1
  fi
  awk '/^<NO/{print "";next} {printf $0","}' ${req_file_name}"this.xml" | sed 's/,$/\n/g' | sed '/^$/d' >${req_file_name}"inv.txt"
  inv_pieces=`cat ${req_file_name}"inv.txt" | wc -l`
#  echo "${inv_pieces}"
...
# return the result of the selection!
  ...
  done <${req_file_name}"inv.txt"
  rm -rf ${req_file_name}"inv.txt"
  rm -rf ${req_file_name}"this.xml"
