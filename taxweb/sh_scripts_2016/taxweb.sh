...
for ((h=0;h<3;h++))
do
  rel=`curl -s -k "${url}/SbsqWW/qrgycx.do?callback=jQuery110206540922571677747_1495246360383&cert=${taxno}&token=${token}&ymbb=${ymbb}&rznf=&_=${rand}"`
  #echo $rel
  echo "token is ${token}"
...
sql_old="SELECT income_month,COUNT(1),SUM(inv_vat) FROM scm.t_scm_vat_main WHERE buyer_taxno=${sqltax} and inv_dedu_result="1" AND income_month=${inv_month};"
...
if [[ "${inv_sql_amount}" = 0 ]]; then inv_sql_vat=0; fi
...
sql_ok="SELECT COUNT(1) FROM scm.t_scm_vat_main WHERE buyer_taxno=${sqltax}"' AND inv_type="0" AND inv_dedu_result="0" and inv_status ="0" and inv_cost > 0 and inv_date >= '"${invdate_q}  and inv_date <= ${invdate_z};"
sql_loseCtl="SELECT COUNT(1) FROM scm.t_scm_vat_main WHERE buyer_taxno=${sqltax}"' AND inv_type="0" AND inv_dedu_result="0" and inv_status ="1" and inv_cost > 0 and inv_date >= '"${invdate_q}  and inv_date <= ${invdate_z};"
sql_cancel="SELECT COUNT(1) FROM scm.t_scm_vat_main WHERE buyer_taxno=${sqltax}"' AND inv_type="0" AND inv_dedu_result="0" and inv_status ="2" and inv_cost > 0 and inv_date >= '"${invdate_q}  and inv_date <= ${invdate_z};"
sql_red="SELECT COUNT(1) FROM scm.t_scm_vat_main WHERE buyer_taxno=${sqltax}"' AND inv_type="0" AND inv_dedu_result="0" and inv_status ="3" and inv_cost > 0 and inv_date >= '"${invdate_q}  and inv_date <= ${invdate_z};"
sql_except="SELECT COUNT(1) FROM scm.t_scm_vat_main WHERE buyer_taxno=${sqltax}"' AND inv_type="0" AND inv_dedu_result="0" and inv_status ="4" and inv_cost > 0 and inv_date >= '"${invdate_q}  and inv_date <= ${invdate_z};"
#echo "sql_ok: ${sql_ok}"
#echo "sql_loseCtl: ${sql_loseCtl}"
#echo "sql_cancel: ${sql_cancel}"
#echo "sql_red: ${sql_red}"
#echo "sql_except: ${sql_except}"
...
((rel_sum=${rel_ok}+${rel_loseCtl}+${rel_cancel}+${rel_red}+${rel_except}))
#echo "rel_ok: ${rel_ok}"
      ...
      do
        ...
        if [ -z "$nc_num" ]; then
          ...
          if [ `echo "${nc_num} - ${rel_loseCtl}" | bc ` -eq 0 ]; then
            ...
            echo "失控 webtax : ${nc_num}  mysql : ${rel_loseCtl} different!";
          fi
        fi
      done
    ;;   
